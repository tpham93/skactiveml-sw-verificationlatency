import unittest
import numpy as np
from verification_latency import (
    ForgettingWrapper,
...
class QueryTests(ABC):
    ...
    def test_query_param_tcandidates(self):
        # tcandidates needs to be a list of numeric type and same size as ty_cand
        self.assertRaises(
            ValueError,
            self.delay_wrapper.query,
            candidates=self.candidates,
        ...
        self.assertRaises(
            ...
            tX=self.tX,
            ty=self.ty,
            tcandidates=self.tcandidates[1:],
            ty_cand=self.ty_cand,
            acquisitions=self.acquisitions,
            sample_weight=self.sample_weight,
            return_utilities=False,
    ...
    def test_query_param_ty_cand(self):
        # ty_cand needs to be a list of numeric type and same size as tcandidates
        self.assertRaises(
            ValueError,
            self.delay_wrapper.query,
            candidates=self.candidates,
...
class TestForgettingWrapper(unittest.TestCase, QueryTests):
    def setUp(self):
        ...
        self.candidates = np.array([self.X[2]])
